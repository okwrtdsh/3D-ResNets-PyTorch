...
from spatio_temporal_transforms import Coded, Averaged, OneFrame, ToTemporal, ToRepeat, Reconstruct, SpatioTemporal
...
if __name__ == '__main__':
    ...
    if not opt.no_val:
        ...
        elif opt.compress == 'spatio_temporal':
            spatio_temporal_transform = Compose([
                SpatioTemporal(opt.spatial_compress_size, use_cv2=opt.use_cv2),
            ])
            temporal_transform = TemporalCenterCrop(opt.sample_duration)
        elif opt.compress == 'temporal':
            spatio_temporal_transform = Compose([
        ...
        elif opt.compress == 'reconstruct':
            temporal_transform = TemporalCenterCrop(opt.sample_duration)
            spatio_temporal_transform = Compose([
                Coded(opt.mask_path),
                Reconstruct(opt.mask_path),
            ])
        else:
            spatio_temporal_transform = None
            temporal_transform = TemporalCenterCrop(opt.sample_duration)
