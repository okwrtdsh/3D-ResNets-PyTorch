...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, sample_size=1, sample_duration=16, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
        super().__init__()
        self.exp = Exposuref(t=sample_duration, c=1, s=8)
        ...
        #     bias=False)
        self.conv1 = PatternConv(
            1, self.inplanes,
            kernel_size=7, stride=2, padding=3,
            pattern_size=(8, 8), bias=False)
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        for i, (name, param) in enumerate(model.named_parameters()):
            if 'fc' in name:
                continue
            for (prev_name, prev_param) in state_dict.items():
                if name == prev_name:
                    if param.data.shape == prev_param.data.shape:
